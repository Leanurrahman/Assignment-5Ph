...
const loadAllIssues = () => {
    ...
    .catch(() => {
        if (loadingSection) loadingSection.classList.add("hidden");
        if (issuesContainer) issuesContainer.innerHTML = "<p class='text-center text-red-500 col-span-full'>Failed to load data.</p>";
    });
};
...
const setupFilterButtons = () => {
    ...
    if (!allBtn || !openBtn || !closedBtn) return;

    const updateButtonStyles = (activeButton) => {
        const buttons = [allBtn, openBtn, closedBtn];
        const inactiveClass = "btn bg-white hover:bg-gray-100 text-gray-600 font-medium px-8 py-2 rounded-md border border-gray-300 shadow-sm min-w-[120px] h-10 transition-colors";
        const activeClass = "btn bg-[#4A00FF] hover:bg-[#3b00cc] text-white font-medium px-8 py-2 rounded-md border-none shadow-sm min-w-[120px] h-10 transition-colors";
...
const setupSearchFunctionality = () => {
    ...
    const performSearch = () => {
        ...
        .then(json => {
            ...
            const countElement = document.getElementById("issues-count");
            if (countElement) countElement.innerText = `${results.length} Issues`;
        })
        .catch(() => {
            if (loadingSection) loadingSection.classList.add("hidden");
            if (container) container.innerHTML = "<p class='text-center text-red-500 col-span-full'>Search failed.</p>";
        });
    };
